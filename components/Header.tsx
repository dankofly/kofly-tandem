    ...
    >
      <div className="container-wide">
        <div className="flex items-center justify-between h-16 lg:h-20">
          ...
          {/* Mobile: KI + Burger
              KI-Button: aria-label enthaelt den sichtbaren Text (WCAG 2.5.3
              Label in Name) und ist lokalisiert. Vorher stand dort auch auf
              /en und /nl hartcodiert "Kofly KI oeffnen". */}
          <div className="flex items-center gap-1 lg:hidden">
            <button
              onClick={() => window.dispatchEvent(new Event("open-chat"))}
              className="relative flex items-center gap-1 pr-1 pl-2.5 py-2 min-h-[44px] text-accent-500 hover:text-accent-400 transition-colors"
              aria-label={`${t("fragMich")}, Kofly KI`}
            >
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true" className="w-5 h-5 animate-[bounce_2s_ease-in-out_infinite]">
              ...
              <span className="text-[9px] font-bold uppercase tracking-wider leading-none text-accent-500">{t("fragMich")}</span>
            </button>
          <button
