import { Link } from "@/i18n/navigation";
import Image from "next/image";
import { getImageUrl } from "@/lib/images-config";
import { getTranslations } from "next-intl/server";
import ScrollReveal from "./ScrollReveal";

export default async function Packages() {
  const t = await getTranslations("Packages");
  const packagesImage = await getImageUrl("packages");

  const packages = [
    {
      name: t("classicName"),
      price: "150",
      note: t("classicNote"),
      features: [t("classicFeature1"), t("classicFeature2"), t("classicFeature3"), t("classicFeature4")],
      highlighted: false,
    },
    {
      name: t("classicMediaName"),
      price: "170",
      note: t("classicMediaNote"),
      features: [t("classicMediaFeature1"), t("classicMediaFeature2"), t("classicMediaFeature3")],
      highlighted: false,
    },
    {
      name: t("premiumName"),
      price: "190",
      note: t("premiumNote"),
      features: [t("premiumFeature1"), t("premiumFeature2"), t("premiumFeature3"), t("premiumFeature4")],
      highlighted: true,
    },
    {
      name: t("thermikName"),
      price: "250",
      pricePrefix: "ab ",
      note: t("thermikNote"),
      features: [t("thermikFeature1"), t("thermikFeature2"), t("thermikFeature3"), t("thermikFeature4")],
      highlighted: false,
    },
  ];

  return (
    <section id="pakete" className="relative py-14 sm:py-16 lg:py-24 overflow-hidden scroll-mt-20">
      {/* Glow orb */}
      <div className="glow-orb glow-orb-accent w-[300px] h-[300px] sm:w-[500px] sm:h-[500px] -bottom-40 left-1/4 animate-glow-pulse" aria-hidden="true" />

      <div className="relative z-10 max-w-6xl mx-auto px-6">
        {/* Header */}
        <ScrollReveal className="text-center mb-10">
          <p className="text-sm tracking-premium uppercase text-accent-500 font-semibold">
            {t("tagline")}
          </p>
          <h2 className="mt-3 text-3xl sm:text-4xl lg:text-5xl font-black text-content-primary tracking-tight">
            {t("title")}
          </h2>
          <div className="mt-5 section-divider" />
        </ScrollReveal>

        {/* Section image */}
        {packagesImage && (
          <ScrollReveal animation="scale-in" className="mb-8 rounded-2xl overflow-hidden border border-edge-faint">
            <Image
              src={packagesImage}
              alt={t("imageAlt")}
              width={1200}
              height={600}
              className="w-full h-auto object-cover"
              loading="lazy"
            />
          </ScrollReveal>
        )}

        {/* Package grid */}
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {packages.map((pkg, i) => (
            <ScrollReveal
              key={pkg.name}
              delay={i * 100}
              className={`relative flex flex-col rounded-2xl p-6 sm:p-8 transition-all duration-400 ${
                pkg.highlighted
                  ? "glass-card border-accent-500/30 shadow-glow-accent lg:-my-4 lg:py-12"
                  : "glass-card card-hover-glow"
              }`}
            >
              {pkg.highlighted && (
                <span className="absolute top-0 left-0 right-0 h-px bg-gradient-to-r from-transparent via-accent-500 to-transparent" aria-hidden="true" />
              )}

              <p className="text-[13px] text-content-subtle font-medium tracking-wide uppercase">
                {pkg.note}
              </p>
              <h3 className="mt-2 text-sm font-bold text-content-strong leading-snug min-h-[2.5rem] whitespace-pre-line">
                {pkg.name}
              </h3>

              <div className="mt-5 flex items-baseline gap-1">
                {pkg.pricePrefix && (
                  <span className="text-sm font-medium text-content-subtle">{pkg.pricePrefix}</span>
                )}
                <span className="text-3xl sm:text-4xl font-black text-content-primary tracking-tight">
                  {"\u20AC"}{pkg.price}
                </span>
                <span className="text-xs text-content-subtle">,00</span>
              </div>

              <ul className="mt-5 space-y-2.5 flex-1">
                {pkg.features.map((f) => (
                  <li key={f} className="flex items-start gap-2.5 text-sm text-content-body font-normal leading-relaxed">
                    <span className="w-1.5 h-1.5 rounded-full bg-accent-500 mt-1.5 flex-shrink-0" aria-hidden="true" />
                    {f}
                  </li>
                ))}
              </ul>

              <Link
                href="/buchen"
                className={`mt-6 block text-center px-6 py-3.5 rounded-lg text-sm font-semibold tracking-wide uppercase transition-all duration-300 min-h-[44px] ${
                  pkg.highlighted
                    ? "bg-accent-500 hover:bg-accent-400 text-white btn-glow"
                    : "border border-edge-secondary hover:border-accent-500/50 text-content-body hover:text-content-primary"
                }`}
              >
                {t("cta")}
              </Link>
              <a
                href={`https://wa.me/436767293888?text=${encodeURIComponent(t("whatsappText", { name: pkg.name, price: pkg.price }))}`}
                target="_blank"
                rel="noopener noreferrer"
                className="mt-2 flex items-center justify-center gap-1.5 text-sm text-[#25D366]/70 hover:text-[#25D366] font-semibold tracking-wide transition-colors py-2 min-h-[44px]"
              >
                <svg className="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                  <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
                </svg>
                {t("whatsappCta")}
              </a>
            </ScrollReveal>
          ))}
        </div>

        {/* Gutschein hint */}
        <ScrollReveal as="p" className="mt-8 text-center text-[13px] text-content-subtle font-normal">
          {t("voucherHint")}{" \u00B7 "}
          <Link
            href="/buchen#gutschein"
            className="text-accent-400 hover:text-accent-500 transition-colors"
          >
            {t("voucherLink")}
          </Link>
        </ScrollReveal>
      </div>
    </section>
  );
}
