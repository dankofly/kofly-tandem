...
const FAQ_TOPICS = ["Experience", "Age", "Height", "Weather"] as const;
const INCLUDED = ["1", "2", "3", "4", "5", "6"] as const;
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    ...
    alternates: {
      canonical: `${SITE_URL}/de/classicflug`,
      languages: {
...
export default async function ClassicflugPage() {
  const t = await getTranslations("Classicflug");
  const locale = await getLocale();
  const r = (key: string) => t.rich(key, rich);

  const breadcrumbs = breadcrumbSchema([
    { name: t("breadcrumbHome"), url: `${SITE_URL}/${locale}` },
    { name: t("breadcrumbCurrent"), url: `${SITE_URL}/${locale}/classicflug` },
  ]);

  const product = packageProductSchema({
    name: "Classicflugpaket Tandemflug Lienz – KOFLY",
  ...
  return (
    <>
      ...
      <section className="py-16 lg:py-24">
        <div className="max-w-3xl mx-auto px-6">
          <h2 className="text-2xl sm:text-3xl font-bold text-content-primary tracking-tight">{t("includedTitle")}</h2>
          <div className="mt-5 section-divider !mx-0" />
          <ul className="mt-8 grid gap-3 sm:grid-cols-2">
            {INCLUDED.map((i) => (
              <li key={i} className="flex items-start gap-3 p-4 glass-card">
                ...
                <span className="text-sm text-content-body font-light">{t(`included${i}`)}</span>
              </li>
            ))}
          </ul>
        </div>
      ...
      <section className="py-16 lg:py-24 bg-surface-secondary">
        <div className="max-w-3xl mx-auto px-6">
          ...
          <div className="mt-8 space-y-4">
            <Link href="/classicflug" className="block glass-card p-5 border-l-2 border-accent-500/40">
              <span className="text-xs tracking-premium uppercase text-accent-500 font-medium">Classic</span>
            ...
            <Link href="/thermikflug" className="block glass-card card-hover-glow p-5">
              <span className="text-xs tracking-premium uppercase text-content-muted font-medium">Thermik</span>
